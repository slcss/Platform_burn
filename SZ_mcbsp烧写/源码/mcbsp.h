...
#ifndef HARD_CACHE_H_
#define HARD_CACHE_H_
...
struct hard_cache {	
	...
	int 	tail;
	int 	size;
	char 	buf[CACHE_SIZE][PACKAGE_MAX_LENGTH];
};



extern int hard_cache_init(void);
extern void hard_cache_exit(void);
extern int get_cache_size(enum cache_type type);

extern int inin_cache(enum cache_type type,u32 *temp);
extern int outout_cache(enum cache_type type, char *temp);
